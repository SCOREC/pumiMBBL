...
{
  int Nel_total = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (pumi_mesh->nsubmeshes-1))->submesh_Nel + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (pumi_mesh->nsubmeshes-1))->Nel_cumulative;
  return Nel_total;
}
...
{
  return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel;
}
...
{
  double global_x_left = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + 0)->x_min;
...
{
  double global_x_right = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (pumi_mesh->nsubmeshes - 1))->x_max;
...
void pumi_BL_elemsize_ON_1D(pumi_mesh_t *pumi_mesh){
  ...
  for (isubmesh=0; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
      int left_Nel = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel;
      ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize = (double*) malloc(left_Nel*sizeof(double));
      ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords = (double*) malloc((left_Nel+1)*sizeof(double));
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + 0) = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + 0) = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min;
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + 1) = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min+((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
      int iCell;
      for (iCell=1; iCell<left_Nel; iCell++){
        *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + iCell) = *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + (iCell-1))*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r;
        *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + (iCell+1)) = *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + (iCell)) + *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + iCell);
      }
    }
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
      int right_Nel = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel;
      ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize = (double*) malloc(right_Nel*sizeof(double));
      ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords = (double*) malloc((right_Nel+1)*sizeof(double));
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + 0) = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0*pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel-1);
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + 0) = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min;
      *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + 1) = *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + 0) + *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + 0);
      int iCell;
...
void pumi_BL_elemsize_OFF_1D(pumi_mesh_t *pumi_mesh){
  ...
  for (isubmesh=0; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
      free(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize);
      free(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords);
    }
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
      free(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize);
      free(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords);
...
double pumi_return_1D_gradingratio(pumi_mesh_t *pumi_mesh, int node){
  ...
  if (node == 0 || node == pumi_mesh->pumi_Nel_total){
    ...
    for (isubmesh=0; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
      ...
      int submesh_left_node = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative;
      int submesh_right_node = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel;

      if (node >= (submesh_left_node + 1) && node <= (submesh_right_node - 1)){

        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & uniform){
          return 1.0;
        }
        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
          return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r;
        }
        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
          return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r;
        }
      ...
      else if (node == ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative){
        double left_elem_size;
        double right_elem_size;
        // On LHS of the node
        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->pumi_flag & rightBL){
          left_elem_size = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->t0;
        }
        else{
          if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->pumi_flag & uniform){
            left_elem_size = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->t0;
          }
          else{
            double t0 = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->t0;
            double r = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->r;
            double Nel = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh-1))->submesh_Nel;
            left_elem_size = t0*pow(r,Nel-1);
          }
        }
        // On RHS of the node
        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->pumi_flag & leftBL){
          right_elem_size = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
        }
        else{
          if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->pumi_flag & uniform){
            right_elem_size = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
            ...
            double t0 = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
            double r = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->r;
            double Nel = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->submesh_Nel;
...
double pumi_return_1D_elemsize(pumi_mesh_t *pumi_mesh, int index, int offset){
  ...
  int isubmesh;
  for (isubmesh=0; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
    int submesh_left_elem = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative;
    int submesh_right_elem = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel-1;

    if (elem >= submesh_left_elem && elem <= submesh_right_elem){

      if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & uniform){
          return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
      }
      if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
          int local_elem = elem - submesh_left_elem;
          if (pumi_mesh->BL_elem_coords_cache_flag){
            return *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + local_elem);
          }
          else {
            double t0 = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
            double r = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->r;
      ...
      if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
        int submesh_right_Nel = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel;
...
double pumi_return_smallest_elemsize(pumi_mesh_t *pumi_mesh){
  ...
  if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
    smallest_elemsize = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
  }
  else{
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
      smallest_elemsize = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
  ...
  for (isubmesh=1; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
    ...
    if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
      new_smallest_elemsize = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
    }
    else{
      if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
        new_smallest_elemsize = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + (isubmesh))->t0;
...
void pumi_locate_submesh_and_cell(pumi_mesh_t *pumi_mesh, double coords, int *submeshID, int *cellID){
    if (pumi_mesh->nsubmeshes == 1){
        *submeshID = 0;
        ...
        for (isubmesh=1; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
            if (coords < ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min){
...
void pumi_update_submesh_and_cell(pumi_mesh_t *pumi_mesh, double coords, int prev_submeshID, int *submeshID, int *cellID){
    if (pumi_mesh->nsubmeshes == 1){
        ...
        *submeshID = prev_submeshID;
        while(coords<((pumi_submesh_t*) pumi_mesh->pumi_submeshes + *submeshID)->x_min){
            *submeshID -= 1;
        }

        while(coords>((pumi_submesh_t*) pumi_mesh->pumi_submeshes + *submeshID)->x_max){
            *submeshID += 1;
        }
    }
    *cellID = pumi_locatecell_fnptr[*submeshID](pumi_mesh, *submeshID, coords);
...
void pumi_update_submesh_and_update_cell(pumi_mesh_t *pumi_mesh, double coords, int prev_submeshID, int prev_cellID, int *submeshID, int *cellID){
    if (pumi_mesh->nsubmeshes == 1){
        ...
        *submeshID = prev_submeshID;
        while(coords<((pumi_submesh_t*) pumi_mesh->pumi_submeshes + *submeshID)->x_min){
            *submeshID -= 1;
            prev_cellID = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + *submeshID)->submesh_Nel - 1;
        }
...
int pumi_locatecell_in_uni(pumi_mesh_t *pumi_mesh, int isubmesh, double coord){
    int icell = (coord - ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
    return icell;
}
...
int pumi_updatecell_in_uni(pumi_mesh_t *pumi_mesh, int isubmesh, int icell, double coord){
    icell = (coord - ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
    return icell;
}
...
void pumi_calc_weights_in_uni(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double coord, int *global_cell, double* weight){
    *weight = (coord - (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0*local_cell))/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
    *global_cell = local_cell + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative;
}
...
int pumi_locatecell_in_leftBL(pumi_mesh_t *pumi_mesh, int isubmesh, double coord){
    int icell = log(1 + (fabs(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min-coord))*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->log_r;
    return icell;
}
...
int pumi_updatecell_in_leftBL_cached(pumi_mesh_t *pumi_mesh, int isubmesh, int icell, double coord){
    while(coord < *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + icell)){
        icell -= 1;
    }

    while(coord > *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + (icell+1))){
        icell += 1;
    }
    return icell;
}
...
int pumi_updatecell_in_leftBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int icell, double coord){
    icell = log(1 + (fabs(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min-coord))*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->log_r;
    return icell;
}
...
void pumi_calc_weights_in_leftBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double coord, int *global_cell, double* weight){
    double r_power_cell = pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,local_cell);
    *global_cell = local_cell + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative;
    *weight = (coord - (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min + (r_power_cell-1.0)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio))/(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0*r_power_cell);
}
...
void pumi_calc_weights_in_leftBL_cached(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double coord, int *global_cell, double* weight){
    *global_cell = local_cell + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative;
//    *weight = (coord - (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_left + (*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->leftBL_elemsize + local_cell) - ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->lBL_t0)/(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->left_r - 1.0)))/(*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->leftBL_elemsize + local_cell));
    *weight = (coord - *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + local_cell))/(*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + local_cell));
}
...
int pumi_locatecell_in_rightBL(pumi_mesh_t *pumi_mesh, int isubmesh, double coord){
    int icell = log(1 + (fabs(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_max-coord))*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->log_r;
    return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel - icell - 1;
}
...
int pumi_updatecell_in_rightBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int icell, double coord){
    icell = log(1 + (fabs(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_max-coord))*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->log_r;
    return ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel - icell - 1;
}
...
void pumi_calc_weights_in_rightBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double coord, int *global_cell, double* weight){
    local_cell = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel - local_cell - 1;
    double r_power_cell = pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,local_cell);
    *global_cell = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel - local_cell - 1 ;
    *weight = 1 - ((((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_max - (r_power_cell-1.0)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio) - coord)/(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0*r_power_cell);
}
...
//    local_cell = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->right_Nel - local_cell - 1;
    *global_cell = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative + local_cell ;
//    *weight = 1 - ((((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_right - (*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->rightBL_elemsize + local_cell) - ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->rBL_t0)/(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->right_r-1.0)) - coord)/(*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->rightBL_elemsize + local_cell));
    *weight = (coord - *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + local_cell))/(*(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + local_cell));
}
...
int pumi_global_cell_ID(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell){
    return (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->Nel_cumulative + local_cell);
}
...
void pumi_calc_node_coords_in_uni(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double *left_node, double *right_node){
    *left_node = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0*local_cell;
    *right_node = *left_node + ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
}
...
void pumi_calc_node_coords_in_leftBL_cached(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double *left_node, double *right_node){
    *left_node = *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + local_cell);
    *right_node = *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_coords + (local_cell+1));
}
...
void pumi_calc_node_coords_in_leftBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double *left_node, double *right_node){
    double r_power_cell = pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,local_cell);
    *left_node = (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_min + (r_power_cell-1.0)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio);
    *right_node = *left_node + r_power_cell*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
}
...
void pumi_calc_node_coords_in_rightBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell, double *left_node, double *right_node){
    local_cell = ((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->submesh_Nel - local_cell - 1;
    double r_power_cell = pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,local_cell);
    *right_node = (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->x_max - (r_power_cell-1.0)/((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r_t0_ratio);
    *left_node = *right_node - r_power_cell*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0;
}
...
double pumi_calc_elem_size_in_leftBL_cached(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell){
    return *(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->BL_elemsize + local_cell);
}
...
double pumi_calc_elem_size_in_leftBL_analytic(pumi_mesh_t *pumi_mesh, int isubmesh, int local_cell){
    double r_power_cell = pow(((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->r,local_cell);
    return (r_power_cell*((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->t0);
}
...
void pumi_initialize_locatecell_and_calcweights_functions(pumi_mesh_t *pumi_mesh){
    ...
    pumi_calc_elem_size_fnptr = malloc(pumi_mesh->nsubmeshes*sizeof(pumi_calc_elem_size_ptr));
    int isubmesh;
    for(isubmesh=0; isubmesh<pumi_mesh->nsubmeshes; isubmesh++){
        if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & leftBL){
        ...
        else if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & rightBL){
        ...
        else if (((pumi_submesh_t*) pumi_mesh->pumi_submeshes + isubmesh)->pumi_flag & uniform){
